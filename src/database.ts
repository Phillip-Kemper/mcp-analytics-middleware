...
export class AnalyticsDatabase {
  ...
  getToolCallStats() {
    return {
      totalCalls: (this.db.prepare('SELECT COUNT(*) as count FROM tool_calls').get() as { count: number }).count,
      ...
      errorRate: (this.db.prepare(`
        ...
        FROM tool_calls
      `).get() as { error_rate: number }).error_rate
    };
  }

  getResourceRequestStats() {
    return {
      totalRequests: (this.db.prepare('SELECT COUNT(*) as count FROM resource_requests').get() as { count: number }).count,
